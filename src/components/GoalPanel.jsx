  ...
  return (
    <div className="rounded-lg p-4 mb-4" style={{ background: '#FFFFFF', border: '1px solid #DCE3DE', boxShadow: '0 1px 3px rgba(0,0,0,0.07)' }}>
      <div className="flex items-center justify-between mb-2">
        <p className="font-semibold text-sm" style={{ color: '#16201C' }}>🎯 목표 달성도</p>
        <p className="font-bold text-lg" style={{ color: scoreColor }}>{result.score}%</p>
      </div>
      <div className="w-full h-2 rounded-full overflow-hidden mb-3" style={{ background: '#E6EEE9' }}>
      ...
      <div className="space-y-1.5">
        {result.results.map(({ goal, matched, covered }) => (
          <div key={goal.id} className="flex items-center justify-between">
            <span className="flex items-center gap-1.5" style={{ fontSize: 12, color: '#16201C' }}>
